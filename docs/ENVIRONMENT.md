...
| `COLI_CONTINUE_ASSISTANT` | `1` (on) | On the OpenAI- and Anthropic-compatible chat endpoints, continue a trailing `assistant` message instead of opening a new turn — the checkpoint's own template with `add_generation_prompt=False`. On by default: a message list ending in a non-empty `assistant` turn continues, the same contract as Anthropic's API. Set `0` to restore the old behavior (append a fresh generation cue). Refused with `tools`/`tool_calls`, and the turn must carry text not ending in whitespace. A family whose renderer has no open-turn shape yet (Kimi K3, framed engine-side) falls through to the old behavior. Unrelated to `COLI_PREFILL_CHUNK`, which is the compute phase. |
